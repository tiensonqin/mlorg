...
module E = struct
  ...
  let rewrite_footnotes =
    Config.add config "rewrite-footnotes" boolean
      "True if footnotes should be handled" true

  let number_heading =
    Config.add config "number-heading" boolean
      "True if headings should be numbered" true

  let heading_format =
    Config.add config "heading-format" string
      "The format used to number the headings" "1.1.1.1.1"

  let footnote_format =
    Config.add config "footnote-format" string
      "The format used to number the footnotes" "1"

  let appendix_format =
    Config.add config "appendix-format" string
      "The format used to number the headings in the appendix" "A.1.1.1.1"

  let footnotes_level =
    Config.add config "footnotes-level" int
      "Displays the footnotes at the end of sections of given level" 2

  let number_heading_format =
    Config.add config "number-heading-format" string
      "The format in which insert the the number in the heading (beginning)"
      ~vars:[make_var "number" "The number of the heading"]
      "$number "

  let number_toc =
    Config.add config "number-toc" boolean
      "True if there should be numbers in the table of contents" false
end
...
let format format numbers = Numbering.update ~trunc:true format numbers

let shall_be_numbered = Document.has_tag "nonumber" %> not

let contains_appendix =
  List.exists (function Custom ("appendix", _, _) -> true | _ -> false)
...
let gather config doc =
  ...
  let compute_number numbers is_appendix =
    format
      (Config.get config
         (if is_appendix then appendix_format else heading_format))
      numbers
  in
  let o =
    object (self)
...
let link s =
  ...
  in
  String.to_list s |> List.map map_char |> String.concat ""

let generate config toc =
  let rec aux numbers l =
    Block.List
      ( List.mapi
          (fun k {name; anchor; children; number} ->
            let number =
              if Config.get config number_toc then Some number else None
            in
            ...
            ; Block.number } )
          l
      , Config.get config number_toc )
  in
  aux [] toc

let clean table footnotes =
  let on f g x y = f (g x) (g y) in
  footnotes
  |> List.filter (snd %> ( <> ) [])
...
let transform config doc =
  ...
  let footnotes =
    Document.footnotes doc |> List.mapi (fun i (a, b) -> (a, (i, b)))
  in
  let footnotes_seen = ref [] in
  let format_footnote k = format (Config.get config footnote_format) [1 + k] in
  let o =
    object (self)
          ...
          let b =
            ...
            :: List.map
                 (fun (name, contents) ->
                   ...
                   Paragraph
                     ( Target (self#make_footnote_label name)
                     :: Superscript [Plain s]
                     :: Plain " " :: contents ) )
                 footnotes_uniq
          in
      ...
      method inline (numbers, toc) =
        ...
        | Footnote_Reference {Inline.name; definition} as x -> (
          ...
          with Not_found ->
            Log.warning
              "Reference to unknown footnote `%s' (It is defined in the same \
               section?)"
              name ;
            x )
      ...
      method heading (numbers, toc) t =
        let () =
          footnotes_seen := !footnotes_seen @ Document.(t.meta.footnotes)
        in
        let ft_contents =
          if
            Config.get config rewrite_footnotes
            && t.Document.level <= Config.get config footnotes_level
          then self#flush_footnotes ()
          else []
        in
        if shall_be_numbered t then
          ...
          let children =
            List.mapi
              (fun k i -> self#heading (numbers @ [1 + k], echildren) i)
              t.Document.children
          in
          let assoc l s = try List.assoc s l with _ -> "" in
          let prefix =
            substitute
              (assoc [("number", entry.number)])
              (Config.get config number_heading_format)
          in
        ...
        else {t with Document.content= t.Document.content @ ft_contents}
      ...
      method document (_, toc) doc =
        ...
        if Config.get config number_heading then
          { doc with
            Document.headings=
              List.mapi
                (fun k i -> self#heading ([1 + k], toc) i)
                doc.Document.headings
          ; Document.beginning=
              self#blocks ([], toc) doc.Document.beginning @ footnotes }
        else {doc with Document.beginning= doc.Document.beginning @ footnotes}
  ...
  in
  o#document ([], ref global_toc) doc

let rec mem_item name toc =
  name = Inline.asciis toc.name || mem name toc.children
