...
module P = struct
  ...
  let dir =
    Config.add config "dir" string
      "The directory where to store images of preprocessed formulas" "lxtpng"

  let latex =
    Config.add config "latex" string "The latex command to invoke"
      "latex -interaction=batchmode"

  let dvipng =
    Config.add config "dvipng" string "The dvipng command to invoke" "dvipng"

  let header =
    Config.add config "header" string "Header to add to the tex files" ""

  let pinline =
    Config.add config "inline" boolean
      "Tell whether inline math should be processed" false

  let pblock =
    Config.add config "block" boolean
      "Tell whether block math (between $$) should be processed" true
...
let compile config texfile =
  Printf.sprintf "%s %s > /dev/null 2> /dev/null" (Config.get config latex)
    (Filename.quote texfile)
  |> Sys.command
  ...
  let int_of_substring s =
    let rec aux k s =
      if length s = 0 then (k, s)
        ...
        | '0' .. '9' ->
            aux
              ((10 * k) + int_of_char (get s 0) - int_of_char '0')
              (triml 1 s)
        | _ -> (k, s)
    in
    aux 0 s
...
let math2png config formulas =
  let list =
    List.map
      (fun s ->
        ( s
        , Filename.concat (Config.get config dir)
            (Digest.to_hex (Digest.string s) ^ ".png") ) )
      formulas
  in
  let pwd = Sys.getcwd () in
  let () = try Unix.mkdir (Config.get config dir) 0o755 with _ -> () in
  ...
  if list = [] then []
    ...
    let () =
      ...
      |> List.iteri (fun k (a, (b, c)) ->
             Sys.rename
               (Printf.sprintf "%s/%d_tmp.png" (Config.get config dir) (1 + k))
               b )
    in
    list
...
let transform conf doc =
  let gather_formulas () =
    let o =
      object (self)
        ...
        method block l =
          function
          | Math b when Config.get conf pblock -> ("$$" ^ b ^ "$$") :: l
  ...
  let formulas =
    ...
    if l <> [] then math2png conf (gather_formulas ()) else []
  in
  let o =
    object (self)
      ...
      method lookup formula =
        ...
        Link
          { url= Complex {protocol= "depth-" ^ string_of_int n; link= s}
          ; label= [Plain formula] }
  ...
  in
  let doc' = if formulas <> [] then o#document () doc else doc in
  doc'
