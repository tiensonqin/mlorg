...
module P = struct
  ...
  let ocamlc =
    Config.add config "ocamlc" string "ocamlc command to use"
      "ocamlfind ocamlc -c -package batteries,mlorg"

  let ocamlopt =
    Config.add config "ocamlopt" string "ocamlopt command to use"
...
struct
  ...
  let compile ?(clean = true) ~file conf =
    ...
    let v = link obj in
    if clean then
      List.iter rm [file; obj; change_ext "cmi" obj; change_ext "o" obj] ;
    v
  ...
  let load_source config source doc =
    let ret ?ml_source file lines number =
      load ?ml_source config lines file number
    in
    ...
    let block ml_source = function
      ...
      | _ ->
          Log.warning "Denoted block is not a source block." ;
          None
    in
    let lines ml_source code =
      ret ?ml_source "<command-line>" (String.nsplit ~by:"\n" code) 1
    in
    try
    ...
    with _ -> (
      if Sys.file_exists source then file source
        ...
        try
          Scanf.sscanf source "%[^:]:%[^:]:%[^\\00]" (fun typ ml_source data ->
              let ml_source =
                if ml_source = "" then None else Some ml_source
              in
              match typ with
              | "block" -> (
                try
                  let (Some b) = Document.find_block_by_name data doc in
                  block ml_source b
                with _ ->
                  Log.error "Invalid block named %s" data ;
                  None )
