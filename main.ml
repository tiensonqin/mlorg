...
(* pure part *)
let with_filename s f = if s = "-" then f stdin else BatFile.with_file_in s f

let with_document opts source f =
  let config = Plugin.global_config opts in
  if source = "-" then f (Document.from_chan ~config "<stdin>" stdin)
  else f (Document.from_file ~config source)
...
let generate backend output opts filename =
  ...
  try
    ...
    let fdout = if output = "-" then IO.stdout else File.open_out output in
...
let man =
  [ `S "DESCRIPTION"
  ; `P
      "$(tname) converts org-mode files into other formats such as LaTeX, \
       HTML, XML, and so on." ]
  @ options
