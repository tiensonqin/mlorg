...
let int =
  let module Int = struct
    ...
    let read s = try Some (int_of_string s) with _ -> None
  end in
...
let boolean =
  let module Boolean = struct
    ...
    let read s =
      if s = "yes" || s = "true" then Some true
...
let string =
  ...
  (module String : SerializableType with type t = string)

(* We use here Delimiters to parse the couples. *)
let table = [('(', (')', false)); ('[', (']', false)); ('"', ('"', true))]

module D = Delimiters.Make (struct
  let table = table
end)

let couple (type v u) (s1 : u serializable) (s2 : v serializable) =
  ...
  let module Couple = struct
    ...
    let description =
      Printf.sprintf "Couple of (%s, %s)" S1.description S2.description
    ...
    let read s =
      ...
      then None
        ...
        | [a; b] -> (
          match (S1.read a, S2.read b) with
  ...
  let module List = struct
    ...
    let read s =
      ...
      then None
        ...
        let sub = BatSubstring.substring s 1 (String.length s - 2) in
        match D.split sub ',' with l -> Some (List.filter_map S.read l)
  end in
...
let parse_keyvalue s =
  try Scanf.sscanf s " %[^=]=%[^\\00]" (fun key v -> (key, v)) with _ ->
    (s, "yes")

let parse_comma s = split ',' s |> List.map parse_keyvalue

type variable = {var_name: string; var_descr: string}
...
type instance =
  {get: 'a. ?vars:(string * string) list -> 'a item -> 'a option; config: t}
...
let get (type u) ?vars instance item =
  ...
  | Some v -> v
  | None ->
      let module M = (val item : Item with type t = u) in
      M.default
...
let add (type u) ?(long = "") ?(vars = []) config name (typ : u serializable)
    description default =
  let module I = struct
    type t = u
...
let append list ({config} as instance) =
  ...
  let compute (type u) (item : u item) vars =
    ...
    | Not_found ->
        ...
        v
    | _ ->
        Log.warning "Value %s is invalid for configuration item %s"
...
let to_man config =
  ...
  |> List.sort (fun i i' -> compare (name i) (name i'))
  |> List.fold_left
       (fun acc i ->
         ...
         let man =
           [ `I
               ( spr "$(b,%s) (type: $(b,%s))" I.name I.T.description
               , esc I.description )
           ; `Noblank
