...
module E = struct
  ...
  let rec inline = function
    ...
    | Plain s -> ws "%s" (escape ["/"; "="; "*"; "_"; "{"; "["; "]"; "}"] s)
    | Emphasis (kind, data) ->
        let s =
          List.assoc kind [(`Bold, "*"); (`Italic, "/"); (`Underline, "_")]
        in
    ...
    | Inline_Call {program; arguments; inside_headers; end_headers} ->
        let arguments =
          List.map (fun (a, b) -> Printf.sprintf "%s=%s" a b) arguments
          |> String.concat ","
        in
        ws "call_%s%s(%s)%s" program
          (wrap_bracket inside_headers)
          (escape ["("; ")"] arguments)
          (wrap_bracket end_headers)
    | Inline.Inline_Source_Block {Inline.language; options; code} ->
        ws "src_[%s]%s%s" language
          (Hd_arguments.to_string options)
          (wrap_brace (Some code))
    | Subscript t -> ws "_{%s}" (inlines t)
    | Superscript t -> ws "^{%s}" (inlines t)
    | Macro (name, args) ->
        ws "{{{%s(%s)}}}"
          (escape ["{"; "}"] name)
          (String.concat ", " (List.map (escape ["{"; "}"]) args))
    | Latex_Fragment (Inline.Math s) -> ws "$%s$" (escape ["$"] s)
  ...
  class orgExporter out =
    ...
    object (self)
      ...
      method block =
        ...
        | Math s -> ws "$$ %s $$\n" s
        | Quote l -> ws "#+begin_quote\n" ; self#blocks l ; ws "#+end_quote\n"
        | With_Keywords (l, b) ->
            List.iter (fun (name, v) -> ws "#+%s: %s\n" name v) l ;
            self#block b
        | Example (_, lines) -> List.iter (ws ": %s\n") lines
        | Src src ->
            let pr_line (a, b) =
              ws "%s" a ;
              ( match b with
              | Some s ->
                  ws " " ;
                  ws "%s" (Printf.sprintf src.ref_format s)
              | None -> () ) ;
              ws "\n"
            in
            ws "#+begin_src %s %s\n" src.language
        ...
        | Footnote_Definition (name, contents) ->
            ws "[%s] " name ; self#inlines contents

      method heading d =
        let iter f = Option.map_default f () in
        for i = 1 to d.level do
          ws "*"
        done ;
        ws " " ;
        ...
        iter (ws "%s ") d.marker ;
        self#inlines d.name ;
        if d.tags <> [] then ws "     :%s:" (String.concat ":" d.tags) ;
        ws "\n" ;
  ...
  module Exp = struct
    let export _ doc out = (new orgExporter out)#document doc
