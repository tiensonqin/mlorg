...
module E = struct
  ...
  let write_header config out doc =
    let vars =
      [ ("classname", Latex.escape_inside (Config.get config classname))
      ; ("packages", "")
  ...
  class beamerExporter config out =
    object (self)
      ...
      method heading toc heading =
        ...
        else super#heading toc heading

      method block toc =
        function
        | Custom (name, opts, blocks)
          when List.mem name
                 ["alert"; "color"; "uncover"; "alt"; "invisible"; "only"] ->
