...
module E = struct
  ...
  class xmlExporter =
    object (self)
      ...
      method inline =
        ...
        | Macro (name, args) ->
            [ Xml.block "macro" ~attr:[("name", name)]
                (List.map
                   (fun v -> Xml.block "parameter" ~attr:[("value", v)] [])
                   args) ]
        ...
        | Link {url; label} ->
            [ Xml.block "link"
                ~attr:[("href", Inline.string_of_url url)]
                (self#inlines label) ]
        | Break_Line -> [Xml.block "break-line" []]
        | Target s -> [Xml.block "target" [Xml.data s]]
        | Cookie (Percent k) ->
            [Xml.block "cookie" ~attr:[("percent", string_of_int k)] []]
        | Cookie (Absolute (k, k')) ->
            [ Xml.block "cookie"
                ~attr:[("done", string_of_int k); ("total", string_of_int k')]
                [] ]
        ...
        | Timestamp (Clock (Stopped t)) ->
            [Xml.block "clock" ~attr:[("stopped", "true")] [self#range t]]
        | Timestamp (Clock (Started t)) ->
            [Xml.block "clock" ~attr:[("stopped", "false")] [self#timestamp t]]
        | Radio_Target s -> [Xml.block "radio-target" [Xml.data s]]
        | Verbatim s -> [Xml.block "verbatim-inline" [Xml.data s]]
      ...
      method time {hour; min} =
        Xml.block "time"
          ~attr:[("hour", string_of_int hour); ("min", string_of_int min)]
          []

      method timestamp {active; date; time; repetition} =
        Xml.block "timestamp"
          ~attr:[("active", if active then "true" else "false")]
          [ self#date date
          ; Option.map_default self#time Xml.empty time
          ; Option.map_default
              (Xml.block "repetition" % (fun x -> [x]) % self#date)
              Xml.empty repetition ]

      method list_item x =
        let number =
          match Option.map Numbering.extract x.number with
          | Some (n :: _) -> Some (string_of_int n)
          | _ -> None
        in
        [ Xml.block "item"
            ~attr:
              ( opt_attr "label" x.number
      ...
      method block =
        ...
        | Heading _ -> []
        | List (l, _) -> [Xml.block "list" (concatmap self#list_item l)]
        | Directive _ -> []
        | Math s -> [Xml.block "math-block" [Xml.data s]]
        | Quote l -> [Xml.block "quote" (self#blocks l)]
        | With_Keywords (l, b) ->
            [Xml.block "with-keywords" ~attr:l (self#block b)]
        | Example (line, l) ->
            [ Xml.block "example"
                ~attr:[("linenumber", string_of_int line)]
                [Xml.data (String.concat "\n" l)] ]
        | Src {linenumber; header_arguments; lines; language} ->
            [ Xml.block "source"
                ~attr:
                  [ ("options", Hd_arguments.to_string header_arguments)
                  ; ("linenumber", string_of_int linenumber) ]
                (List.map
                   (fun (a, b) ->
                     let attr =
                       match b with None -> [] | Some s -> [("ref", s)]
                     in
                     Xml.block ~attr "line" [Xml.data a] )
                   lines) ]
        | Custom (name, opt, contents) ->
            [ Xml.block "custom"
                ~attr:[("name", name); ("options", opt)]
                (self#blocks contents) ]
        | Latex_Environment (name, opts, contents) ->
            [ Xml.block "latex-environment"
                ~attr:[("name", name); ("opts", opts)]
                [Xml.data (String.concat "\n" contents)] ]
        | Drawer (name, c) ->
            [Xml.block "drawer" ~attr:[("name", name)] (self#blocks c)]
        | Property_Drawer _ -> []
        | Table t ->
            let index =
              Option.map_default
            ...
            let contents =
              lmap "rowgroup" (lmap "row" (lmap "cell" self#inlines)) t.rows
            in
            [ Xml.block "table"
                ~attr:(opt_attr "format" t.format)
      ...
      method heading d =
        let mk_list name f l =
          if l = [] then Xml.empty else Xml.block name (List.map f l)
        in
        let attr =
          [("level", string_of_int d.level)]
          @ opt_attr "marker" d.marker
          @ opt_attr "priority" (Option.map (String.make 1) d.priority)
        in
        let children =
          Xml.block "meta"
        ...
        in
        [Xml.block "heading" ~attr children]

      method meta d =
        let couple name (a, b) =
          Xml.block name ~attr:[("name", a); ("value", b)] []
        in
        [ Xml.block "opts" (List.map (couple "opt") d.opts)
        ; Xml.block "directives" (List.map (couple "directive") d.directives)
        ; Xml.block "extensions"
            (List.map (fun a -> Xml.block "ext" ~attr:[("name", a)] []) d.exts)
        ]
  ...
  module D = struct
    let export _ doc out = write out ((new xmlExporter)#document doc)
