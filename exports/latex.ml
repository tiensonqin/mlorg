...
module L = struct
  ...
  let footer = add config "footer" string "The LaTeX footer" "\\end{document}"

  let extraheader = add config "extraheader" string "Extra LaTeX header" ""

  let sections =
    add config "sections" (list string) "The name of the sections"
      ["section"; "subsection"; "subsubsection"; "paragraph"; "subparagraph"]

  let pdfmeta =
    add config "pdf-meta" boolean
  ...
  let write_header config out doc =
    let vars =
      [ ("classname", escape_inside (Config.get config classname))
    ...
    in
    let meta s s' =
      if s' = "" then "" else Printf.sprintf "%s={%s}" s (tex_escape s')
    in
    IO.nwrite out (substitute (assoc vars) (Config.get config header)) ;
    Printf.fprintf out "\\hypersetup{%s,%s}\n"
      (meta "pdftitle" doc.title)
      (meta "pdfauthor" doc.author)

  class latexExporter config out =
    object (self)
      ...
      method footer doc = IO.nwrite out (Config.get config footer)
          ...
          | None -> (
            try
              ...
              with Not_found ->
                ...
                Printf.fprintf out "}"
            with Not_found ->
              Log.warning "Reference to undefined footnote: %s" name ) )
        | Entity e ->
            if not e.latex_mathp then Printf.fprintf out "%s" e.latex
            else Printf.fprintf out "$%s$" e.latex
        | Latex_Fragment (Inline.Math s) ->
            Printf.fprintf out "$%s$" (escape ["$"] s)
        | Latex_Fragment (Command (name, "")) -> Printf.fprintf out "\\%s" name
        | Latex_Fragment (Command (name, option)) ->
            Printf.fprintf out "\\%s{%s}" name (tex_escape option)
        | Verbatim s -> Printf.fprintf out "\\texttt{%s}" (tex_escape s)
        | Target s -> Printf.fprintf out "\\label{%s}" (Toc.link s)
        | Link {url; label} -> (
          ...
          | Search title, _ ->
              ...
              if Toc.mem title toc then
                ...
                Printf.fprintf out "[%s]"
                  (snd (String.replace ~str:title ~sub:"%20" ~by:" ")) ;
              Printf.fprintf out "{" ;
      ...
      method document toc d =
        self#header d ; self#_document toc d ; self#footer d
    end
  ...
  module D = struct
    ...
    and default_filename = change_ext "tex"
  end
