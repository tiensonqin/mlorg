...
let footnote_reference_parser parse rest =
  ...
  | Some contents ->
      if
        try
          ignore (int_of_string contents) ;
          true
        with _ -> false
      then Some ([Footnote_Reference {name= contents; definition= None}], rest)
      else
...
let inline_source_block_parser _ rest =
  ...
  match code with
  | None -> None
  | Some code -> Some ([Inline_Source_Block {language; options; code}], rest)
...
let subscript_parser, superscript_parser =
  ...
  in
  (gen "_" (fun x -> Subscript x), gen "^" (fun x -> Superscript x))
...
let timestamp_parser _ rest =
  let handle f rest =
    let rest =
      if Substring.length rest = 0 then rest
  ...
  in
  let timestamp, rest =
    match until_space (fun _ -> false) rest with
    ...
    | "CLOCK:", rest ->
        let a, rest =
          try_range
            (fun x -> Clock (Stopped x))
            (fun x -> Clock (Started x))
