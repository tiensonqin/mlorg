...
let get_marker context s =
  ...
  if List.mem word context.Org_context.markers then (Some word, rest)
  else (None, s)

let get_priority s =
  let module String = BatSubstring in
  let word, rest = BatSubstring.splitl (( <> ) ' ') (BatSubstring.trim s) in
  if String.size word = 4 && word.[0] = '[' && word.[1] = '#' && word.[3] = ']'
  then (Some word.[2], rest)
  else (None, s)

let get_title s =
  let module String = BatSubstring in
  let parse_inl =
    Org_inline.parse % BatSubstring.to_string % BatSubstring.trim
  in
  let title, tags =
    BatSubstring.splitr
      (fun c -> c = ':' || Char.is_letter c || Char.is_digit c)
      s
  in
  if
    (not (BatSubstring.is_empty tags))
    && tags.[0] = ':'
    && tags.[String.size tags - 1] = ':'
  then (parse_inl (BatSubstring.trim title), Some tags)
  else (parse_inl s, None)

let get_tags s =
  let s = BatSubstring.triml 1 (BatSubstring.trimr 1 s) in
  BatSubstring.split_on_char ':' s |> List.map BatSubstring.to_string
...
let is_start {line; context} =
  try Some (context, parse_heading context line) with _ -> None
