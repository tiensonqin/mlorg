...
type state =
  { items: Block.list_item list  (** The items gathered so far *)
  ; last_line_empty: bool  (** Was the last line empty ? *)
  ; current: string list  (** The contents of the current item *)
  ; number: int option  (** Its number *)
  ; format: string  (** Its format *)
...
let compute_number ordered format default =
  if not ordered then None
    ...
    match Option.map Numbering.extract format with
    | Some [n] -> Some n
    | _ -> Some default
...
let parse_first_line s =
  ...
  let parse_fmt s =
    if s.[0] = '@' then Some (String.sub s 1 (String.length s - 1)) else None
  in
  ...
  let next =
    ...
    else if String.length s > 2 && (s.[0] = '+' || s.[0] = '-') && s.[1] = ' '
    then Some (1, false)
    else
      try Scanf.sscanf s "%d.%n" (fun _ k -> Some (k, true)) with _ -> None
  in
  match next with
  | None -> None
  | Some (k, ordered) -> (
      ...
      | Some (marker, rest) -> (
          let b, rest =
            if marker = " " then (Some false, rest)
...
let rec skip k indent string =
  ...
  else if string.[k] = ' ' then skip (k + 1) (Option.map pred indent) string
  else if indent = None then (Some k, Some (String.lchop ~n:k string))
  else (indent, None)

let parse_line st
    {Org_automaton.line; Org_automaton.parse; Org_automaton.context} =
  if line = "" then
    if st.last_line_empty then
      ...
      ( context
      , Org_automaton.Partial
          {st with last_line_empty= true; current= "" :: st.current} )
  else
