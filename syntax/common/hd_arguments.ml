...
type value = Value of string | Call of string * (string * value) list
...
let rec parse_var sub =
  if length sub = 0 then failwith "empty var"
    ...
    let value, rest =
      take_while (fun c -> Char.is_digit c || Char.is_latin1 c) sub
    in
    ...
    if length rest > 0 && get rest 0 = '(' then
      ...
      | Some (contents, rest) ->
          ...
          ( Call (to_string value, List.map (all %> parse_binding) args)
          , trim rest )
    else (Value (to_string value), rest)

and parse_binding sub =
  try
    Scanf.sscanf (to_string sub) "%[^=] = %[^\n]" (fun a b ->
        (String.trim a, fst (parse_var (all b))) )
  with _ -> failwith ("Invalid binding (" ^ to_string sub ^ ")")

and parse_binding_opt sub =
  try
    Scanf.sscanf (to_string sub) "%[^=] = %[^\n]" (fun a b ->
        (String.trim a, Some (fst (parse_var (all b)))) )
  with _ -> (to_string sub, None)

let table' = [('(', (')', false)); ('"', ('"', true)); (' ', (' ', false))]
...
let whd_ table l x =
  if l = [] then x
    ...
    try List.assoc (List.hd l) table with Not_found ->
...
let options =
  [ ( "var"
    , update_vars @|@ fun l y -> List.map (all %> parse_binding_opt) l @ y )
  ; ( "results"
    , (fun x f -> f x)
      @|@ fun l x ->
      let handle_word x = function
        ...
        | _ -> x
      in
      List.fold_left handle_word x l )
  ...
  ; ( "tangle"
    , update_tangle
      @|@ whd_map (function "no" -> `No | "yes" -> `Yes | s -> `File s) )
  ...
  ; ( "session"
    , update_session
      @|@ fun l _ -> if l = [] then Some "" else Some (List.hd l) )
  ; ( "noweb"
    , update_noweb
      @|@ whd_
            [ ("no", `No)
...
let to_string x =
  String.concat " "
    (List.map
       (fun (a, b) -> Printf.sprintf ":%s %s" a (String.concat " " b))
       x.arguments)
