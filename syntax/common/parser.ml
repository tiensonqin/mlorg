...
*)
  ...
  let rec handle_lines myself lines context_ all previous state =
    let aux ?context number =
      let ctx = Option.default context_ context in
      handle_lines myself lines (C.set_number ctx number) all
    in
  ...
  *)
    ...
    | Some line, None ->
        if line = "" then aux (number + 1) previous None
        else
    ...
    | Some line, Some state -> (
        ...
        | context, Automaton.Done (bl, b) ->
            let number =
              if not b then ( Enum.push lines line ; number ) else number + 1
            in
        ...
        | context, Automaton.Partial st' -> (
          try
            ...
            Some
              (C.set_number context (number + 1), previous, Some state', blocks)
          with _ -> aux (number + 1) previous (Some st') ) )
