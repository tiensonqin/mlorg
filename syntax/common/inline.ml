...
and inline_source_block =
  {language: string; options: Hd_arguments.t; code: string}
...
and latex_fragment = Math of string | Command of string * string
...
class ['a] mapper =
  object (self)
    method inline (v : 'a) =
      function
      | Emphasis (a, b) -> Emphasis (a, self#inlines v b)
      | Footnote_Reference ref ->
          Footnote_Reference
            {ref with definition= Option.map (self#inlines v) ref.definition}
      | Link l -> Link {l with label= self#inlines v l.label}
      ...
      | ( Macro _ | Radio_Target _ | Verbatim _ | Cookie _ | Timestamp _
        ...
        | Target _ | Break_Line | Export_Snippet _ | Entity _ ) as x ->
          x
...
class ['a] folder =
  object (self)
    method inline (v : 'a) =
      ...
      | Link l -> self#inlines v l.label
      | Subscript t -> self#inlines v t
      | Superscript t -> self#inlines v t
      | Macro _ | Radio_Target _ | Verbatim _ | Cookie _ | Timestamp _
       |Plain _ | Inline_Call _ | Inline_Source_Block _ | Target _
       |Latex_Fragment _ | Break_Line | Export_Snippet _ | Entity _ ->
          v
...
let rec ascii = function
  ...
  | Emphasis (_, t) | Subscript t | Superscript t -> asciis t
  | Macro _ | Radio_Target _ -> ""
  | Verbatim s -> s
  | Cookie _ -> ""
  | Timestamp _ -> ""
  | Target _s -> ""
