...
type 'a parser =
  (string -> 'a list) -> Substring.t -> ('a list * Substring.t) option
...
let rec run_parsers plain parsers string =
  ...
  let rec skip_word s =
    match Substring.length s with
    | 0 -> 0
    | _ -> (
      match Substring.get s 0 with
  ...
  let rec aux start acc substring =
    ...
    if Substring.is_empty substring then List.rev (push acc start current)
    else
      let rec try_parsers = function
        | [] ->
            let lg = skip_word substring in
            aux start acc (Substring.trim (Substring.triml lg substring))
        | t :: q -> (
          try
...
let inside table delim rest =
  ...
  if Substring.is_empty rest then (None, rest)
    ...
    match D.enclosing_delimiter rest delim with
    | Some (c, d) -> (Some c, d)
    | None -> (None, rest)

let inside_force table delim rest =
  match inside table delim rest with Some c, d -> (c, d) | _ -> failwith ""

let see s rest =
  let before, after = Substring.split_at (Batteries.String.length s) rest in
  if Substring.to_string before = s then after else raise (Failure "")
