...
type meta =
  ...
  ; deadlines: Timestamp.t list  (** The deadlines appearing in the heading *)
...
type t =
  ...
  ; beg_meta: meta  (** The timestamp present in the beginning *)
  ; exts: string list  (** The extensions used by the documents *)
...
let expand_macros =
  Config.add config "expand-macros" Config.boolean "Expand macros" true
...
class ['a] mapper =
  object (self)
    ...
    method document (v : 'a) d =
      { d with
        headings= map self#heading v d.headings
      ; beginning= self#blocks v d.beginning }

    method heading v h =
      { h with
        name= self#inlines v h.name
...
class ['a] folder =
  object (self)
    ...
    method heading v h =
      self#inlines
        (fold self#heading (self#blocks v h.content) h.children)
        h.name
  end
...
class virtual ['a] bottomUp =
  object (self)
    ...
    method heading h =
      ...
      self#combine (a :: b :: List.map self#heading h.children)
  end
...
class virtual ['a, 'b] bottomUpWithArg =
  object (self)
    ...
    method heading arg h =
      ...
      self#combine (a :: b :: List.map (self#heading arg) h.children)
  end
...
let collect =
  ...
  let collector =
    object (self)
      ...
      method block meta =
        function
        | Block.Property_Drawer p -> {meta with properties= p @ meta.properties}
        | Block.Footnote_Definition (name, def) ->
            if meta.footnotes |> List.exists (fst %> ( = ) name) then
              ...
              orphans := (name, def) :: !orphans ;
              meta )
        | block -> super#block meta block
      ...
      method inline meta =
        ...
        | Footnote_Reference {name} -> (
          try
            { meta with
              footnotes= (name, List.assoc name !orphans) :: meta.footnotes }
          with Not_found -> {meta with footnotes= (name, []) :: meta.footnotes}
          )
...
let handle_directives doc =
  { doc with
    exts= (try words (List.assoc "extensions" doc.directives) with _ -> [])
  ; opts= List.filter (fun (s, _) -> String.exists s ".") doc.directives }

let directives =
  List.filter_map (function Block.Directive (a, b) -> Some (a, b) | _ -> None)

let opts =
  List.filter_map (function
    | Block.Directive (a, b) when String.contains a '.' -> Some (a, b)
    | _ -> None )
...
let substitute_macro directives name arguments =
  let macros =
    directives
    |> List.filter (fst %> String.uppercase %> ( = ) "MACRO")
    |> List.map (snd %> String.split ~by:" ")
  in
  try List.assoc name directives with Not_found -> (
    try
...
let from_blocks ?config:conf filename blocks =
  ...
  let blocks =
    if Config.get config expand_macros then
  ...
  let leave_heading heading c =
    let heading =
      { heading with
        children= List.rev heading.children; content= c; meta= collect c }
    in
  ...
  let directives =
    let o =
      object
        ...
        method block l =
          function
          | Block.Directive (a, b) -> (a, b) :: l | x -> super#block l x
      end
    in
    o#blocks [] blocks
  in
  ...
  let rec aux has_contents heading blocks =
    let up_contents c = if has_contents then heading.content else c in
    match look_for_heading blocks with
    | all, None, _ -> (leave_heading heading (up_contents all), [])
        ...
        if k > heading.level then
          let child, rest =
            aux false
              ...
              ; children= []
              ; tags= h.Block.tags
              ; marker= h.Block.marker
              ; priority= h.Block.priority
              ; meta= empty_meta }
              rest
          in
          aux true
            { heading with
              children= child :: heading.children; content= up_contents start
            }
            rest
        else
          (leave_heading heading (up_contents start), Block.Heading h :: rest)
  in
...
let from_chan ?config filename channel =
  BatIO.lines_of channel |> Org_parser.parse |> snd
...
let prop_val name h =
  try Some (List.assoc name h.meta.properties) with Not_found -> None

let prop_val_ name h = List.assoc name h.meta.properties
...
let blocks_by_keywords pred doc =
  let folder =
    object (self)
      ...
      method block acc =
        function
        | Block.With_Keywords (l, t) when pred l -> self#block (t :: acc) t
        | x -> super#block acc x
    end
  in
  folder#document [] doc
...
let find_block_by_name name doc =
  match
    blocks_by_keywords
      (fun l -> try List.assoc "NAME" l = name with _ -> false)
      doc
  with
  | x :: _ -> Some x
  | _ -> None
...
let current_clocked_item doc =
  let o =
    object (self)
      ...
      method heading v h =
        match h.meta.current_clock with
        | Some c -> Some h
...
let current_clocking_time doc =
  Option.map clocking_time (current_clocked_item doc)
...
let heading ?(timestamps = []) ?(ranges = []) ?(scheduled = [])
    ?(deadlines = []) ?(properties = []) ?(footnotes = []) ?(clocks = [])
    ?current_clock ?father ?priority ?(anchor = "") ?(content = []) ?marker
    ?(tags = []) ?(children = []) ~level name =
  let inline =
    Inline.(
      List.concat
        ...
        ; ( match current_clock with
          | Some x -> [Timestamp (Clock (Started x))]
          | _ -> [] ) ])
  in
  let body =
