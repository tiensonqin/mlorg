...
module Exporters = struct
  ...
  let config () =
    Config.concat (List.map (fun e -> (name e, config e)) (get ()))
end
...
let global_config parameters = Config.make (get_global_interface ()) parameters

let eprint_config_descr = get_global_interface %> Config.prettyprint stderr

let options_man = get_global_interface %> Config.to_man
